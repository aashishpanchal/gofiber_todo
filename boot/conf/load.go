...
type Config struct {
	...
	GO_ENV      string `validate:"required,oneof=development production"`
	CORS_ORIGIN string `validate:"required"`
	VERSION     string `validate:"required"`
	// Rate limiter
	RATE_LIMIT_WINDOWS int `validate:"required,gt=0"`
	RATE_LIMIT_MAX_REQ int `validate:"required,gt=0"`
	// Database
	DB_URL string `validate:"required"`
	// Jwt config
	JWT_ACCESS_EXP  int64 `validate:"required,gt=0"`
	JWT_REFRESH_EXP int64 `validate:"required,gt=0"`
	// Body parser limit
	BODY_LIMIT uint64    `validate:"required,gt=0"`
...
func Init() {
	once.Do(func() {
		...
		Env = &Config{
			...
			START_TIME:         time.Now(),
			IS_DEV:             GO_ENV == DEVELOPMENT,
			BODY_LIMIT:         getEnvByte("BODY_LIMIT"),
			JWT_ACCESS_EXP:     getEnvMs("JWT_ACCESS_EXP", "5m"),
			JWT_REFRESH_EXP:    getEnvMs("JWT_REFRESH_EXP", "24d"),
			RATE_LIMIT_MAX_REQ: getEnvInt("RATE_LIMIT_MAX_REQ", 100),
			RATE_LIMIT_WINDOWS: int(getEnvMs("RATE_LIMIT_WINDOWS", "15m")),
		}
